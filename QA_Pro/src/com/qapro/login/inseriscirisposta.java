...
public class inseriscirisposta extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(session == null){
			...
			String insertAnswerSql = "INSERT INTO answer "
					+ "(text,insert_date,is_active,responder_id, question_id)"
					   +" VALUES (?,?,?,?,?)";
			String getEmailquestion = "SELECT (email) FROM user WHERE user_id = ?";
			try {
					...
					try {
						   ...
						   if ((rs != null) && rs.next())  {
							   ...
							   System.out.println (rs.getString(1));
						        String subject = "Un nuovo commento su fewchart";
						        String content = "hai ricevuto un nuovo commento su fewcharts!";
					...
					try {
						   pstmt = connection.prepareStatement("SELECT (id) FROM question WHERE id = ?");
