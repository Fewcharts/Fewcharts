...
public class inseriscirisposta extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		if (session == null) {
			...
			int questionIdInt = Integer.parseInt(request
			...
			if (useridLong == null) {
				...
				try {
					...
					preparedStatement = connection
							.prepareStatement(insertAnswerSql);
					...
					try {
						...
						if ((rs != null) && rs.next()) {
							...
							try {
								Email.sendEmail(host, port, user, pass,
										MailAsker, subject, content);
								System.out
										.println("The e-mail was sent successfully");
							} catch (Exception ex) {
								ex.printStackTrace();
					...
					try {
						pstmt = connection
								.prepareStatement("SELECT (id) FROM heroku_f7be13520b27e62.question WHERE id = ?");
