		...
		try {
			...
			if (rs1.next()) {
				...
				if (validation == 0) {
					...
					request.getSession()
							.setAttribute("message1",
									"Please activate your membership a mail must be sent!");
					response.sendRedirect("login.jsp");

					System.out.println("Please activate your membership");
				} else {
					System.out
							.println("adesso fisso la session con member info e la user id e poi dovresti andare su memberhome");
